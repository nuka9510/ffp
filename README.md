...
FFP는 **FrankenPHP**에 최적화된 커스텀 PHP 프레임워크입니다.

## 주요 특징
- **FrankenPHP 워커 모드(Worker Mode) 최적화**: 고성능 상주형 프로세스 실행 지원.
- **커스텀 MVC 아키텍처**: 가볍고 유연한 모델-뷰-컨트롤러 구조.
- **라우팅 및 인터셉터 지원**: 강력한 라우팅 시스템과 미들웨어(인터셉터) 기능 제공.
- **통합 핸들링**: HTTP 요청과 CLI 명령을 동일한 진입점에서 효율적으로 처리.
